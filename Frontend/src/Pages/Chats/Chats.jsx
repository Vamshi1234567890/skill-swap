...
const Chats = () => {
  ...
  const handleChatClick = async (chatId) => {
    try {
      ...
      // toast.success(data.message); // <-- Remove this line
    } catch (err) {
      console.log(err);
      if (err?.response?.data?.message) {
        toast.error(err.response.data.message);
  ...
  const sendMessage = async (e) => {
    try {
      ...
      if (attachedFile) {
        ...
        response = await axios.post("/message/sendMessageWithFile", formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      } else {
        response = await axios.post("/message/sendMessage", { chatId: selectedChat.id, content: message });
      }
            ...
            {selectedChat && (
              <div
                style={{
                  ...
                  display: "flex",
                  alignItems: "center",
                  flexDirection: "column", // stack input and preview vertically
                  background: "#fff",
                }}
              >
                ...
                {attachedFile && (
                  <div style={{ display: "flex", alignItems: "center", marginBottom: "5px", width: "100%" }}>
                    ...
                    <span style={{ color: "#3BB4A1", marginRight: "10px" }}>📎 {attachedFile.name}</span>
                    <button
                      style={{
                        background: "red",
                        color: "#fff",
                ...
                <div style={{ display: "flex", alignItems: "center", width: "100%" }}>
                  ...
                  <label style={{ marginRight: "10px", cursor: "pointer" }}>
                    <span role="img" aria-label="attach" style={{ fontSize: "22px" }}>📎</span>
                    <input
                      type="file"
                      onChange={(e) => {
                        const file = e.target.files[0];
                        setAttachedFile(file);
                        setAttachedFilePreview(file ? URL.createObjectURL(file) : null);
                      }}
                      style={{ display: "none" }}
                    />
                  </label>
