...
const Chats = () => {
  ...
  const joinVideoCall = (roomId) => {
    // Clean up previous Zego instance if exists
    if (zegoRef.current) {
    ...
    setActiveRoomId(roomId);
    window.currentCallStartTime = callStartTime;
  ...
  const startScreenShare = async () => {
    ...
    zp.joinRoom({
      ...
      console.log("✅ Successfully joined Zego room for screen share:", roomId);
    }).catch((error) => {
      console.error("❌ Failed to join Zego room for screen share:", error);
